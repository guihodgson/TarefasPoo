...
public class Batalha {
    ...
    public static boolean iniciar(InputHandler inputHandler, Heroi heroi, ArrayList<Inimigo> inimigos) {
        final double tempoPainel = 0.6;
        final double tempoPadrao = 0.25;
        ...
        while(heroi.estaVivo() && !inimigos.isEmpty()) {
            ...
            if (!nValidoEntre(nCartaSelecionada, 0, heroi.mostrarDeck().size())) {
                BatalhaVisual.exibirErro("Valor Invalido.", inputHandler);
                continue;
            }

            if (nCartaSelecionada == heroi.mostrarDeck().size()) {
            ...
            if (!heroi.podeGastarEnergia(cartaSelecionada.getCusto())) {
                BatalhaVisual.exibirErro("Energia Insuficiente.", inputHandler);
                continue;
            }

            if (cartaSelecionada.getAlvo() == AlvoCarta.UM_ALVO) {
                int nInimSelecionado = inputHandler.selecionar(listarInimigos(inimigos), tempoPadrao, true, "Voltar.");

                if (!nValidoEntre(nInimSelecionado, 0, listarInimigos(inimigos).size())) {
                    BatalhaVisual.exibirErro("Valor Invalido.", inputHandler);
                    continue;
                }

                if (nInimSelecionado == listarInimigos(inimigos).size()) {
                    continue;
                }
            ...
            if (cartaSelecionada.getAlvo() == AlvoCarta.GLOBAL) {
                heroi.usarCartaNDeck(nCartaSelecionada, inimigos.toArray(new Inimigo[0]));
            }
            
            if (cartaSelecionada.getAlvo() == AlvoCarta.USO_PROPRIO) {
            ...
            heroi.atualizarEfeito("ataque");
            inputHandler.clear();
            InputHandler.imprimirBonito(cartaSelecionada.getArte().getArte(), tempoPadrao);
